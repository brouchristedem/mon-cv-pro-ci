...
export default function Template03({ cv }: { cv: CVData }) {
  ...
  return (
    <div className="w-full min-h-full text-slate-700 p-12 font-sans text-[13px] leading-relaxed" style={{ background: cv.couleurFond }}>
              ...
              {section.items.map((item) => (
                <div key={item.id} className="break-inside-avoid">
                  <div className="flex justify-between items-baseline">
                    <span className="font-medium">{bulletTitle(section.type, item.titre)}</span>
